...
pub fn decode(object: &[u8], key: &ContainerKey) -> Result<DecodedContainer> {
    let header = Header::parse(object)?;

    // The associated data is the header exactly as it appears in the object.
    let (associated_data, body) = object.split_at(Header::LEN);
    ...
    let chunk_size =
        usize::try_from(header.chunk_size.get()).map_err(|_| Error::InvalidChunkSize)?;
    let mut writer = StreamWriter::new(
        meta.entries.iter().map(|entry| entry.size).collect(),
        meta.pad_len,
        expected_len,
    );

    let mut chunks = &body[meta_len..];
    if chunks.is_empty() {
    ...
    while !chunks.is_empty() {
        ...
        let message_len = chunk_size + TAG_LEN;
