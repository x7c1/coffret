...
fn tampering_with_the_container_id_fails_decryption() {
    ...
    object[8] ^= 0x01;
    assert_eq!(decode(&object, &key()), Err(Error::AuthenticationFailed));
}
...
fn reordering_chunks_fails_authentication() {
    ...
    assert_eq!(decode(&reordered, &key()), Err(Error::AuthenticationFailed));
}
...
fn dropping_the_final_chunk_fails_authentication() {
    ...
    assert_eq!(decode(&truncated, &key()), Err(Error::AuthenticationFailed));
}
