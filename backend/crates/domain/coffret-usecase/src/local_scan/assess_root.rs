//! Asking one mapped root which [`RootState`] it is in.
//!
//! The question is here and the answer is its own type, so the verdict can be
//! named after what it is rather than after the function that reaches it.
...
/// (spec: EP-12).
///
/// This is the question; [`RootState`] is the answer it gives, and what the
/// walk carries for each mapping once it has been asked.
///
...
pub(super) async fn assess_root(
...
mod tests {
    ...
    async fn a_root_holding_only_the_management_area_still_reads_as_empty() {
        ...
        let state = assess_root(&fs, &moved())
            .await
    ...
    async fn a_root_holding_a_name_that_folds_to_the_reserved_one_is_reported() {
        ...
        let Err(refused) = assess_root(&fs, &moved()).await else {
