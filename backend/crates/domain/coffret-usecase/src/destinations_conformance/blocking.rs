...
///
/// The folder the walk stopped at travels with the refusal because it is what
/// a person acts on: the Entry Path says which file went unplaced, and this
/// says which name to go and look at.
///
...
pub async fn a_symlink_on_the_way_blocks_the_reach_and_names_it(fixture: &DestinationsUnderTest) {
    ...
    let refused = fixture
        ...
        .reach(
            fixture.root(),
            Some(&expected),
            &components(&["link", "authorized_keys"]),
        )
        .await
        .err()
    ...
    match refused {
        DescentError::Blocked { stopped_at } => assert_eq!(
            stopped_at, blocked,
            "the folder the walk stopped at is what there is to look at",
        ),
...
pub async fn a_file_where_a_folder_must_be_blocks_the_reach(fixture: &DestinationsUnderTest) {
    ...
    match refused {
        DescentError::Blocked { stopped_at } => assert_eq!(stopped_at, blocked),
        other => panic!(
