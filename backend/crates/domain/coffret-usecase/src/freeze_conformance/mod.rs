...
//! bucket.
//!
//! Two things have to be arranged that no flow produces. A store that records
//! which objects a run touched, because "an existing Pack was left alone" is a
//! claim about an absence that nothing the flow returns can carry. And a
//! committed Keyring recording a key as lost, which is written by hand for the
//! reason a commit refuses to invent one — losing a key is not something a
//! commit does (spec: KL-7); that one is borrowed from the fetch suite rather
