...
//! bucket.
//!
//! Four things have to be arranged that no flow produces. A store that records
...
//! (spec: KL-7). A catalog that watches every spool announcement against the
...
//! Pack spool behind that no row calls whole. And a catalog that shortens a local
//! file as a Pack is announced, because the window in which a file can stop being
//! the file the scan measured opens after the scan and closes when the encoder
//! reads it — inside one call, where nothing planted beforehand reaches. Two of
//! the four are borrowed — from the fetch suite and from the sync suite — rather
...
//! starts at a folder — but every one of them is in the in-memory disk the
//! fixture makes rather than on a directory the backend hands it. It is behind
//! the `conformance` feature so that only test targets pay for it.
...
/// two empty catalogs, and two empty folders to run the case against, or
