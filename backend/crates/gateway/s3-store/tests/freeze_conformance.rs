...
//! a failure harder to place. The folders are temporary directories of this
//! target's, because a freeze starts at a real filesystem whichever provider the
//! Library is on; where the ciphertext waits is the fixture's own in-memory
//! spool, which `spool_conformance` holds to its own contract.
//!
...
/// Hands the suite an empty Library, two empty catalogs, and two empty
