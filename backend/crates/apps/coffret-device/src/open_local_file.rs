...
impl OpenLibrary {
    ...
    /// everything the EP-9 translation refuses on the way to the file: `Index`
    /// where the catalog could not be read, `UnmappedEntryPath` where no mapping
    /// of this device reaches the path, `UnmaterializablePath` where one does
    /// and no file here can stand for it (spec: EP-2, EP-4), and
    ...
    pub async fn open_local_file(&self, path: &EntryPath) -> Result<Option<LocalFile>> {
        ...
        let place = local_place_of(self.index.as_ref(), path)
            .await
            .map_err(|cause| Error::LocalFileNotOpened { cause })?;
