...
impl Folder {
    ...
    ///
    /// The last separator and nothing cleverer: an Entry Path's only logical
    /// separator is `/` (spec: EP-2), so everything before the last one is the
    /// folder and a path with none of them sits directly in the Library root.
    pub fn holding(entry: &EntryPath) -> Self {
        Self(
            entry
                .as_str()
                .rsplit_once('/')
                .map(|(folder, _)| EntryPath::nfc(folder)),
        )
    }
...
mod tests {
    ...
    fn a_folder_is_everything_before_an_entrys_last_separator() {
        assert_eq!(
            Folder::holding(&EntryPath::nfc("albums/2026/spring.jpg")).as_str(),
            "albums/2026",
        );
        assert_eq!(
            Folder::holding(&EntryPath::nfc("albums/cover.png")).as_str(),
            "albums",
        );
    }
    ...
    fn an_entry_with_no_separator_is_in_the_library_root() {
        let root = Folder::holding(&EntryPath::nfc("notes.txt"));
