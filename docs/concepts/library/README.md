...
- A scan reports an Entry as deleted locally only if this device itself had
  **materialized** it — uploaded or fetched it into a mapped folder — and it is
  ...
  rewrites the rest (spec: EP-10). Reporting one also requires the mapped root
  to be available, so an unplugged disk or an unmounted share is reported as an
  unavailable root rather than read as an emptied folder (spec: EP-12).
